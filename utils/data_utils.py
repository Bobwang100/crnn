...
def process_box(boxes, labels, img_size, class_num, anchors):
    ...
    for i, idx in enumerate(best_match_idx):    # value of idx maybe 0 1 2 3 4 5 6 7 8
        # idx: 0,1,2 ==> 2; 3,4,5 ==> 1; 6,7,8 ==> 0   feature_map_group
        feature_map_group = 2 - idx // 3
        # scale ratio: 0,1,2 ==> 8; 3,4,5 ==> 16; 6,7,8 ==> 32
        ratio = ratio_dict[np.ceil((idx + 1) / 3.)]
        x = int(np.floor(box_centers[i, 0] / ratio))
        y = int(np.floor(box_centers[i, 1] / ratio))
        k = anchors_mask[feature_map_group].index(idx)           # anchors_mask = [[6, 7, 8], [3, 4, 5], [0, 1, 2]]
        c = labels[i]
